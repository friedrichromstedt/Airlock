...
3 - Flüssigkeit links: 0 - keine
                       1 - welche
4 - Menge der gepumpten Fluessigkeit pro Frame (Schleife a 1)
...
PumpRightOut: // nur wenn Local(0)=left2right Local(1)=0 [siehe GotoPumpXXX]
	...
	SetVar(1,0);
	ExtractMaterialAmount(0,19,Var(0),Local(4));
	while(LessThan( SetVar(1,Sum(Var(1),1)) , Local(4) ))
...
PumpRightIn: // nur wenn Local(0)=left2right Local(2)=1 [siehe GotoPumpXXX]
	...
	SetVar(1,0);
	ExtractMaterialAmount(55,15,Var(0),Local(4));
	while(LessThan( SetVar(1,Sum(Var(1),1)) , Local(4) ))
